...
(defn init []
    ...
    (items/make-item :name "control pad" :desc "A control pad mounted on the wall." :flags #{:static})
    (items/make-item :name "airlock" :desc "Is airtight when closed." :flags #{:door})
    (items/make-item :name "atmosphere scrubber" :desc "This machine removes co2 and other harmful gasses from the air." :flags #{:machine} :state {:condition 90 :power 0})
    (items/make-item :name "generator" :desc "This machine creates power." :flags #{:machine} :state {:condition 80 :power 100})
    (items/make-item :name "screwdriver" :desc "A screwdriver." :flags #{:tool :sharp :phillips})

    (repeatedly 12 #(humans/make-human)))



(map :UID (vals @(:r DATA)))

(map :exits (vals @(:r DATA)))

(get-by-tag :cargo-bay)
